  ...
  static async improveWebsiteAndSave() {
    ...
    try {
      ...
      while (urlsQueue.length > 0) {
        ...
        let aiResult;
        let improvedHtml;
        /* Try to use AI to improve the webpage. This operation may 
           fail, in which case the original HTML will be used. */
        try {
          aiResult = await HDLmAI.openRouterImproveWebpageV1(currentUrl,
                                                             ...
                                                             '',
                                                             '');
          improvedHtml = HDLmWebsiteImprover.getHtmlFromAiResult(aiResult, currentHtml);
        }
        /* If AI improvement fails, use the original HTML as the 
           improved HTML. */
        catch (errorObj) {
          console.error(errorObj);
