/**
 * HDLmExtensionBothPopup short summary.
 *
 * HDLmExtensionBothPopup description.
 *
 * @version 1.0
 * @author Peter
 */
"use strict";
/* The HDLmExtensionBothPopup class is not used to create any objects.
...
class HDLmExtensionBothPopup {
  ...
  static addToContextMenu(browserName) {
    ...
    if (browserName == 'Firefox') {
      ...
      browser.menus.create({
        ...
        contexts: ["all"]
      }, HDLmExtensionBothPopup.onCreatedBack);
  ...
  static async getActiveDomNodeObjectChrome() {
    /* Check if we are running in a browser extension environment. If this
  ...
  static onClickedContent(event) {
    if (('target' in event) == false)
      HDLmExtensionBothPopup.contentEventTarget = null;
    else
      HDLmExtensionBothPopup.contentEventTarget = event.target;
  }
  ...
  static onInstalledBack(details) {
    ...
    if (details.reason == "install" ||
      ...
      HDLmExtensionBothPopup.addToContextMenu(browserName);
    }
  }
  ...
  static onMessageBack(request, sender, sendResponse) {
    ...
    if (request.itemId == 'Build/Modify Rules') {
      if (browserName == 'Firefox')
        browser.windows.create({ url: "HDLmPopup.html", type: "popup", height: 750, width: 1250 });
      else
        chrome.windows.create({ url: "HDLmPopup.html", type: "popup", height: 750, width: 1250 });
    }
  }
  /* This method is the message event handler. This method is invoked
     when a message event occurs. This method does all of the work needed
    ...
    try {
      ...
      switch (itemId) {
        ...
        case 'Build/Modify Rules': {
          ...
          let browserName = HDLmHtml.getBrowserName();
		      /* Send a message to the background script telling the
		         background script to open (create) a browser extension 
             window */
          if (browserName == 'Firefox')
        ...
        /* Check if the caller is asking for the JSON for the current
           ...
           At present, it comes from the browser extension window 
           created by one of the inline editors. */
        case 'Get JSON For Current Element': {
          ...
          if (browserName == 'Firefox')
            ...
            domNode = HDLmExtensionBothPopup.contentEventTarget;
        ...
        /* Check if the caller is asking for the node identifier for the current
           ...
           it comes from the browser extension window created by one of the inline
        ...
        /* Check if the caller is asking for window (not Windows) information.
           ...
           it comes from the browser extension window created by one of the 
           inline editors.  */
        ...
        case 'Hide HTML Changes': {
          ...
          HDLmExtensionBothPopup.resetStyleSheetEnablement(titleValue, disabledStatus);
        ...
        case 'Show HTML Changes': {
          ...
          HDLmExtensionBothPopup.resetStyleSheetEnablement(titleValue, disabledStatus);
          sessionStorage.setItem(titleValue + 'Disabled', disabledStatusFalse);
          break;
        }        
  ...
  static startup() {
    ...
    if (curEnv == 'content') {
      if (browserName == 'Firefox') {
        browser.runtime.onMessage.addListener(HDLmExtensionBothPopup.onMessageContentOneTime);
        let useCaptureTrue = true;
        document.addEventListener("click", HDLmExtensionBothPopup.onClickedContent, useCaptureTrue);
      }
      if (browserName == 'Chrome') {
        /* console.log('In HDLmExtensionBothPopup.startup'); */
        /* console.log(chrome); */
        ...
        if ((typeof chrome) != 'undefined' &&
          ...
          chrome.runtime.onMessage.addListener(HDLmExtensionBothPopup.onMessageContentOneTime);
        }
        document.addEventListener('contextmenu', HDLmExtensionBothPopup.onClickedContent);
      }
    }
    ...
    if (curEnv == 'background') {
      ...
      if (browserName == 'Firefox') {
        ...
        browser.menus.onClicked.addListener(HDLmExtensionBothPopup.onClickedBack);
		    /* Add the required event listener for messages. This event runs
		       in a background script. */
        browser.runtime.onMessage.addListener(HDLmExtensionBothPopup.onMessageBack);
      }
      /* Check if the current browser is Chrome */
      if (browserName == 'Chrome') {
        chrome.runtime.onInstalled.addListener(HDLmExtensionBothPopup.onInstalledBack);
        let createError = false;
        try {
	        /* Add the required event listener for clicks. This event runs
             in a background script. */
          chrome.contextMenus.onClicked.addListener(HDLmExtensionBothPopup.onClickedBack);
          /* Add the required event listener for messages. This event runs
		         in a background script. */
          chrome.runtime.onMessage.addListener(HDLmExtensionBothPopup.onMessageBack);
        }
        catch (errorObj) {
          createError = true;
...
HDLmExtensionBothPopup.startup();
