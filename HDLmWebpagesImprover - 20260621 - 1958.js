...
const helpTextWpsWIV1 = { 'url': 'Enter the URL of the website you want to improve. Press Enter to fetch and validate the website URL.', 'directory': 'Enter a directory where webpages will be saved', 'improve': 'Click to improve and save all discovered webpages for this website.' };
const openRouterChatTemplatesWpsWIV1 = { 'context': 'You are an expert at improving webpages to increase conversion rates and revenue.\n' + 'Copy everything from the old HTML to the generated HTML by default unless a change is made.\n' + 'Return complete HTML for each page.\n', 'webpageServer': 'Please improve the following HTML to increase conversion rates ' + 'and revenue.\n' + 'Return the complete improved HTML.\n' + '\n' + 'HTML:\n' + '{{html}}\n' };
const openRouterResponseFormatTypeJsonObjectWpsWIV1 = { 'type': 'json_object' };
const openRouterResponseJsonSchemaImproverWpsWIV1 = { 'type': 'json_schema', 'json_schema': { 'name': 'webpages_improver_response', 'description': 'Response containing complete improved HTML and optional improvements list', 'strict': true, 'schema': { 'type': 'object', 'properties': { 'improvedHtml': { 'type': 'string' }, 'improvements': { 'type': 'array', 'items': { 'type': 'object', 'properties': { 'What': { 'type': 'string' }, 'Why': { 'type': 'string' }, 'Hash': { 'type': 'string' } }, 'required': ['What', 'Why', 'Hash'], 'additionalProperties': false } } }, 'required': ['improvedHtml'], 'additionalProperties': false } } };
class HDLmWebpagesImprover {
  ...
  static buildWebUiElement() {
    ...
    let urlInputElement = HDLmReactSix.buildSingleLineInputWLabel('Enter URL (of website) to improve', 'https://www.example.com', 'urlInputWpsWIV1', urlKeyDown, urlChange, HDLmWebpagesImprover.urlInputCurrentValue, true, { marginTop: '0px' }, { marginTop: '0px' }, { display: 'block', marginBottom: '0px' });
    let improveDisabled = HDLmWebpagesImprover.urlValidated == false || HDLmWebpagesImprover.improvingInProgress;
  ...
  static async checkServerStatus() {
    ...
    let extraInfo = '';
    try {
      await HDLmAJAX.runAJAX(requestType, requestAJAXAsyncTrue, urlStr, userid, password, httpType, extraInfo);
      return true;
    }
    catch (errorObj) {
      console.error(errorObj);
  ...
  static async improveWebsiteAndSave() {
    if (HDLmWebpagesImprover.improvingInProgress)
      return;
    if (HDLmWebpagesImprover.urlValidated == false || HDLmWebpagesImprover.originalUrl == null || HDLmWebpagesImprover.originalHtml == null) {
    ...
    try {
      ...
      while (urlsQueue.length > 0) {
        ...
        let improvedHtml;
        try {
          aiResult = await HDLmAI.openRouterImproveWebpageV1(currentUrl, currentHtml, '', useAIVersionWpsWIV1, openRouterChatTemplatesWpsWIV1, openRouterResponseFormatTypeJsonObjectWpsWIV1, openRouterResponseJsonSchemaImproverWpsWIV1, '', '');
      ...
      console.error(errorObj);
      HDLmWebpagesImprover.displayErrorMessage('An error occurred while improving the website: ' + errorObj.message);
    }
    HDLmWebpagesImprover.improvingInProgress = false;
    HDLmWebpagesImprover.forceReRender();
  }
  ...
  static async saveAllImprovedPages(rootDirectoryHandle, improvedHtmlMap) {
    for (let [urlStr, htmlText] of improvedHtmlMap.entries()) {
      ...
      if (directoryParts.length > 0)
        targetDirectoryHandle = await HDLmWebpagesImprover.getDirectoryHandleForPath(rootDirectoryHandle, directoryParts);
  ...
  static visibilityChangeAdd() {
    window.addEventListener('visibilitychange', function(event) { HDLmWebpagesImprover.visibilityChangeDone(event); });
  }
  static visibilityChangeDone(event) {
    ...
    if (document.visibilityState === 'hidden' && HDLmWebpagesImprover.visibilityChangeHiddenCount > 1 && HDLmWebpagesImprover.visibilityChangeHiddenCount > HDLmWebpagesImprover.visibilityChangeVisibleCount) {
